    ...
    <View style={styles.articleContainer}>
      ...
      <View style={styles.articleInfo}>
        ...
        <View style={styles.quantityContainer}>
          <TouchableOpacity
            onPress={() =>
              onQuantityChange(item.id, Math.max(1, item.selectedQuantity - 1))
            }
          ...
          <TouchableOpacity
            onPress={() =>
              onQuantityChange(
                item.id,
                Math.min(item.quantity, item.selectedQuantity + 1)
              )
            }
            style={styles.quantityButton}
          >
...
//'initialisation du formulaire de paiement
  ...
  const STRIPE_PUBLIC_KEY =
    "pk_test_51PfNmyRpKgZkfjqiZU6RXcDkGN4tjVTxY5TA9twzE48MEUMQe8fQojaXd7wJWUaSbRg2jgHmprVUWBGvQ8v8b41K00NeOYreRk"; 
  ...
  //hook useStripe pour accéder aux fonctions nécessaires à l'intégration de Stripe
  ...
  useEffect(() => {
    const initializeStripe = async () => {
      ...
      const { error } = await initStripe({
        publishableKey:
          "pk_test_51PfNmyRpKgZkfjqiZU6RXcDkGN4tjVTxY5TA9twzE48MEUMQe8fQojaXd7wJWUaSbRg2jgHmprVUWBGvQ8v8b41K00NeOYreRk",
          merchantIdentifier: 'merchant.com.TERROTERRO',
      });
      if (error) {
...
//Initialise Stripe lors du montage du composant
  ...
  //Initialise le formulaire de paiement et le réinitialise chaque fois que le panier change
  ...
  const handleSuccessfulPayment = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorResponse.error || "Erreur lors de la mise à jour des quantités"
        );
      }
      ...
      if (
        ...
        alert(
          "Problème de connexion réseau. Le paiement a été effectué mais nous n'avons pas pu mettre à jour les quantités. Veuillez contacter le support."
        );
      } else {
        alert(
          `Erreur lors de la mise à jour des quantités : ${error.message}. Veuillez contacter le support.`
        );
      }
    }
  };
